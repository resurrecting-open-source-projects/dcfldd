...
static char *verify_file = NULL;
static int verify_fd;
static FILE *verify_log;
...
static uintmax_t vskip_records = 0;
...
static uintmax_t vr_partial = 0;

/* Number of full blocks read. */
static uintmax_t r_full = 0;
static uintmax_t vr_full = 0;
...
static int do_hash, do_verify, do_status = 1;
...
static const ssize_t update_thresh = 256;


/* Md5 global data */
static MD5_CTX MD5_total_context;
static MD5_CTX MD5_window_context;
static MD5_CTX MD5_vtotal_context;
static MD5_CTX MD5_vwindow_context;
...
static SHA1Context SHA1_vtotal_context;
static SHA1Context SHA1_vwindow_context;
...
static SHA256_CTX SHA256_vtotal_context;
static SHA256_CTX SHA256_vwindow_context;
...
static SHA384_CTX SHA384_vtotal_context;
static SHA384_CTX SHA384_vwindow_context;
...
static SHA512_CTX SHA512_vtotal_context;
static SHA512_CTX SHA512_vwindow_context;
...
typedef struct
{   /* this struct stores the context pointers
     * associated with a particular stream to
     * be hashed (i.e. input or verify streams)
     */
    char *hash_name;
    void *window_context;
    void *total_context;
    off_t bytes_in_window;
    off_t bytes_in_total;
    void *hashstr_buf;
} hashobj_t;

static hashobj_t input_hash;
static hashobj_t verify_hash;
...
struct hashtype hashops[] =
{
    {"MD5",
     ...
     &MD5_vwindow_context,
     &MD5_vtotal_context,
     (void (*)(void *)) MD5Init,  /* I'm not sure how to avoid
...
static inline void quit(int code)
{
    cleanup ();
    exit(code);
}
 ...
 * this format.
 */
static uintmax_t parse_integer(const char *str, int *invalid)
{
    uintmax_t n;
    char *suffix;
...
static char *make_pattern(char *pattern)
{
...
static void replicate_pattern(char *pattern, char *buffer, size_t size)
{
    size_t i;
...
/* If bs= was given, both `input_blocksize' and `output_blocksize' will
   ...
   bs= was not given, so make sure two buffers are used. */
    ...
    if (do_verify)
        do_hash++;  /* you gotta' hash if you want to verify, baby */
    ...
    if (do_hash) {
        ...
        input_hash.hash_name = hashop->name;
        verify_hash.hash_name = hashop->name;

        input_hash.window_context = hashop->window_context;
        input_hash.total_context = hashop->total_context;
        verify_hash.window_context = hashop->vwindow_context;
        verify_hash.total_context = hashop->vtotal_context;

        input_hash.hashstr_buf = malloc(hashop->hashstr_buf_size);
        verify_hash.hashstr_buf = malloc(hashop->hashstr_buf_size);
        
        hashinit = hashop->init; /* can't avoid these compiler warnings either,
                                  * casts don't do the trick */
        hashupdate = hashop->update;
        hashfinal = hashop->final;
        hashstr_buf = hashop->hashstr_buf;
        hashstr_buf_size = hashop->hashstr_buf_size;
        
        /* a note about hashstr_buf: there is currently a buf for
         * input_hash and verify_hash as well as a global buf.
         * the global buf is used for normal dd operations and
         * the input_hash and verify_hash ones are used in verify.
         */
    }

    /* make sure selected options make sense */
    if (output_file != NULL && verify_file != NULL) {
        fprintf(stderr,"Please select either an output file or a verify file, not both.");
        usage(1);
    }
}
...
static void translate_buffer(unsigned char *buf, size_t nread)
{
    unsigned char *cp;
    size_t i;
...
static unsigned char *swab_buffer(unsigned char *buf, size_t *nread)
{
...
static int buggy_lseek_support(int fdesc)
{
...
static void skip(int fdesc, char *file, uintmax_t records, size_t blocksize,
                 unsigned char *buf)
{
...
static void display_windowhash(char *hash, off_t windowlen)
{
...
static void display_totalhash(hashobj_t *hashobj)
{
    hashfinal(hashobj->total_context, hashstr_buf);
    fprintf(hash_log, "Total: %s\n", (char *) hashstr_buf);
}

static void hash_update_buf(void *buf, size_t len, hashobj_t *hashobj)
{
    if (hash_windowlen != 0) {
        hashupdate(hashobj->window_context, buf, len);
        hashobj->bytes_in_window += len;
    }
    hashupdate(hashobj->total_context, buf, len);
    hashobj->bytes_in_total += len;
}

static void hash_update(void *buf, size_t len, hashobj_t *hashobj)
{
    size_t left_in_window = hash_windowlen - (hashobj->bytes_in_window);

    if (hashobj->bytes_in_total == 0)
        hashinit(hashobj->total_context);

    if (hash_windowlen == 0)
        hash_update_buf(buf, len, hashobj);
    else {
        if (hashobj->bytes_in_window == 0)
            hashinit(hashobj->window_context);
        
        if (len >= left_in_window) {
            hash_update_buf(buf, left_in_window, hashobj);
            hashfinal(hashobj->window_context, hashstr_buf);
            display_windowhash(hashstr_buf, hash_windowlen);
            window_beginning += hash_windowlen;
            hashobj->bytes_in_window = 0;
            hash_update(buf + left_in_window, len - left_in_window, hashobj);
        } else 
            hash_update_buf(buf, len, hashobj);
    }
}
...
/*
 * Returns 1 if a hash mismatch is found, 0 otherwise
 */
static int verify_update(void *ibuf, void *vbuf, size_t ilen, size_t vlen, 
                          hashobj_t *ihash, hashobj_t *vhash)
{
    size_t left_in_window = hash_windowlen - (ihash->bytes_in_window);
    
    if (ihash->bytes_in_total == 0)   /* .bytes_in_total is static so
                                       * it is set to 0 initially */
        hashinit(ihash->total_context);
    if (vhash->bytes_in_total == 0)
        hashinit(vhash->total_context);

    if (hash_windowlen == 0) {
        hash_update_buf(ibuf, ilen, ihash);
        hash_update_buf(vbuf, vlen, vhash);
    } else {
        if (ihash->bytes_in_window == 0) {
            hashinit(ihash->window_context);
            hashinit(vhash->window_context);
        }

        if (ilen >= left_in_window || vlen >= left_in_window) {
            hash_update_buf(ibuf,
                            ilen > left_in_window ? left_in_window : ilen,
                            ihash);
            hash_update_buf(vbuf,
                            vlen > left_in_window ? left_in_window : vlen,
                            vhash);
            hashfinal(ihash->window_context, ihash->hashstr_buf);
            hashfinal(vhash->window_context, vhash->hashstr_buf);

            if (0 != memcmp(ihash->hashstr_buf, vhash->hashstr_buf,
                            hashstr_buf_size))
            {
                fprintf(verify_log, "%llu - %llu: Mismatch\n",
                        (unsigned long long int) window_beginning,
                        (unsigned long long int) (window_beginning + hash_windowlen));
                return 1;
            }

            window_beginning += hash_windowlen;

            ihash->bytes_in_window = 0;
            vhash->bytes_in_window = 0;

            verify_update(ibuf + left_in_window, vbuf + left_in_window,
...
static void hash_remainder(hashobj_t *hashobj)
{
    if (!do_hash)
        return;
    if (hash_windowlen > 0 && hashobj->bytes_in_window > 0) {
        hashfinal(hashobj->window_context, hashstr_buf);
        display_windowhash(hashstr_buf, hashobj->bytes_in_window);
    }
}

static void verify_remainder(hashobj_t *ihash, hashobj_t *vhash)
{
    if (hash_windowlen > 0 && ihash->bytes_in_window > 0) {
        hashfinal(ihash->window_context, ihash->hashstr_buf);
        hashfinal(vhash->window_context, vhash->hashstr_buf);

        if (0 != memcmp(ihash->hashstr_buf, vhash->hashstr_buf, hashstr_buf_size))
            fprintf(verify_log, "%llu - %llu: Mismatch\n",
                    (unsigned long long int) window_beginning,
                    (unsigned long long int) (window_beginning + hash_windowlen));        
    }
...
static void time_left(char *secstr, size_t bufsize, int seconds)
{
...
static int dd_verify(void)
{
    ...
    while (1)
    {
        ...
        /* under current configuration, sync and noerror are always set
         * with verify mode so we don't need to worry about them
         * having different amounts of data.
         */
        left_in_window = hash_windowlen - input_hash.bytes_in_window;
        mismatch = verify_update(ibuf, vbuf, i_nread, v_nread,
                                 &input_hash, &verify_hash);

        if (i_nread != v_nread || (mismatch && i_nread < left_in_window)) {
            size_t winlen;

            if (input_hash.bytes_in_window > verify_hash.bytes_in_window)
                winlen = input_hash.bytes_in_window;
            else
                winlen = verify_hash.bytes_in_window;
            
            fprintf(verify_log, "%llu - %llu: Mismatch\n",
                    (unsigned long long int) window_beginning,
                    (unsigned long long int) (window_beginning + winlen));
            mismatch = 1;
        }
     ...
     */
    if (!mismatch) {  
        verify_remainder(&input_hash, &verify_hash);
        hashfinal(input_hash.total_context, input_hash.hashstr_buf);
        hashfinal(verify_hash.total_context, verify_hash.hashstr_buf);

        if (0 != memcmp(input_hash.hashstr_buf, verify_hash.hashstr_buf, hashstr_buf_size)) {
            size_t winlen;
            ...
            fprintf(verify_log, "%llu - %llu: Mismatch\n",
                    (unsigned long long int) window_beginning,
                    (unsigned long long int) (window_beginning + winlen));
            fprintf(verify_log, "Total: Mismatch\n");
        } else
            fprintf(verify_log, "Total: Match\n");
    } else
        fprintf(verify_log, "Total: Mismatch\n");
...
static int dd_copy(void)
{
    ...
    while (1) {
        ...
        if (do_hash)
            hash_update(ibuf, n_bytes_read, &input_hash);
    ...
    if (do_hash) {
        hash_remainder(&input_hash);
        display_totalhash(&input_hash);
...
static off_t get_dev_size(int fd) 
{
    off_t num_sectors = 0;
    ...
    if (output_file != NULL) {
        ...
        if (seek_records != 0 && !(conversions_mask & C_NOTRUNC)) {
            ...
            if (fstat(STDOUT_FILENO, &stdout_stat) != 0)
                fprintf(stderr, "%s: cannot fstat %s", strerror(errno),
                        output_file);
