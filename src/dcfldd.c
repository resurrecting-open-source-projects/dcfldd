 ...
 * Portions Copyright (c) 2001 DoD Computer Forensics Lab
 * By Nicholas Harbour
 * Based upon GNU dd, which comes as part of the GNU fileutils
...
#define AUTHORS "Nicholas Harbour, Paul Rubin, David MacKenzie, and Stuart Kemp"
...
/* Md5 global data */
static MD5_CTX total_context;
static MD5_CTX window_context;
...
scanargs (int argc, char **argv)
{
  ...
  for (i = optind; i < argc; i++)
	...
	parse_conversion (val);
        ...
        if (pattern == NULL) {
             error (0, 0, _("invalid hex pattern: %s"), quote (val));
             quit (1);
        }
	   ...
	   if (hash_log == NULL)
		error(1, errno, _("opening %s"), quote (val));
      }
...
static void
display_windowhash (char *hash)
{
     fprintf(hash_log, "%llu - %llu: %s\n", (unsigned long long int)window_beginning,
	     (unsigned long long int)(window_beginning + hash_windowlen), hash);
}

static void
display_totalhash (void)
{
     MD5Sprint(&total_context, msgbuf);
...
{
     if (hash_windowlen != 0) {
	  MD5Update(&window_context, buf, len);
	  bytes_in_window += len;
     }
     MD5Update(&total_context, buf, len);
     bytes_in_total += len;
}

static void hash_update (void *buf, size_t len)
{
     off_t left_in_window = hash_windowlen - bytes_in_window;

     if (!do_hash)
	  return;
     if (bytes_in_total == 0)
	  MD5Init(&total_context);

     if (hash_windowlen == 0)
	  hash_update_buf(buf, len);
     else {
	  if (bytes_in_window == 0)
	       MD5Init(&window_context);
	  if (len >= left_in_window) {
	       hash_update_buf(buf, left_in_window);
	       MD5Final(&window_context);
	       MD5Sprint(&window_context, msgbuf);
	       msgbuf[33] = '\0';
	       display_windowhash(msgbuf);
  ...
  while (1)
    {
      ...
      if (do_status && w_full % update_thresh == 0 && w_full != 0)
      {
	   ...
	   off_t total_mb = total_bytes / 1048000;
	   ...
	   if (probe == PROBE_NONE || probed_size == 0)
		...
		off_t probed_mb = probed_size / 1048000;
